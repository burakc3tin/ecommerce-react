...
import { setBrandSelectedFilters } from '../redux/slices/productSlice';

export default function BrandProduct() {
  const [checkedFilters, setCheckedFilters] = useState([]);
  const [modelName, setModelName] = useState('');
  const dispatch = useDispatch();
  const selectedBrandFilters = useSelector((state) => state.products.selectedBrandFilters);
  ...
  const handleFilterChange = (filter) => {
    ...
    dispatch(setBrandSelectedFilters(updatedFilters));
  };
  ...
  const filteredCheckboxes = ['Hyundai', 'Honda', 'Ferrari', 'Lamborghini'].filter((checkbox) =>
          ...
          {filteredCheckboxes.map((checkbox) => (
            <div key={checkbox}>
              <input
                ...
                checked={selectedBrandFilters.includes(checkbox)}
              />
